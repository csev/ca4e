/**
 * WASM Execution Engine - ES Module Version
 * 
 * This version uses ES modules and integrates with the pure JavaScript WAT compiler.
 */

import WatCompilerESM from './wat-compiler-esm.js';
...
export class WasmExecutor {
    constructor() {
        ...
        this.compiler = new WatCompilerESM();
    }
    ...
    async executeWasmText(wasmText) {
        try {
            ...
            // Parse WASM text format and convert to binary using ES module compiler
    ...
    async parseWasmText(wasmText) {
        // Use the ES module WAT compiler instead of PHP service
        try {
            console.log('Compiling WAT using ES module compiler:', wasmText.substring(0, 100) + '...');
            
            const wasmBinary = await this.compiler.compileWatToWasm(wasmText);
            
            // Store the compiled WASM for hex display
            this.lastCompiledWasm = wasmBinary;
