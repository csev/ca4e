...
Up to this point, digital systems have been built from logic gates, storage elements, and
clocked circuits. These components make it possible to store values and perform
arithmetic, but they do not yet explain how a general-purpose machine can follow a
sequence of instructions. That capability emerges when computation is organized around a
central processing unit (CPU) that repeatedly executes a simple control loop.
\index{CPU}

This chapter introduces a complete, working processor model and shows how machine
instructions drive its behavior. Programs are examined not as abstract algorithms, but as
concrete patterns of bits stored in memory that directly control hardware.
\index{machine code}
...
The CDC6504 emulator used in this book models a processor inspired by early
microprocessors, with a small number of registers and a compact instruction set. Despite
its simplicity, it contains all of the essential elements found in modern CPUs.
\index{CDC6504}
...
Registers are small storage locations located directly inside the CPU. They are much
faster to access than memory and are used to hold intermediate results during computation.
\index{registers}
...
\index{fetch-decode-execute cycle}
...
Machine instructions are sequences of bits, but long binary strings are difficult for
humans to read. Hexadecimal notation groups bits into sets of four, making memory contents
easier to inspect and write.
\index{hexadecimal}
...
Assemblers, debuggers, and emulators commonly display memory using hexadecimal for this
reason.
\index{assembler}
...
Some instructions operate directly on registers, while others reference memory. Common
...
Characters are stored as numeric codes, such as ASCII values. A string is simply a
sequence of character codes stored in memory.
\index{ASCII}
